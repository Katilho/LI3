...
TupleU vusers() {
    FILE *fp = fopen ("saida/users-final-g3.csv" , "r");
    char line[8000];
    ...
    int *follower_list, *following_list; 
        ...
        for (int i=0; ( fgets(line, 8000, (FILE*)fp) ); i++) {
...
TupleC vcommits() {
    FILE *fp = fopen ("saida/commits-final-g3.csv" , "r");
    char line[8000];
    char *string, *found, *aux;
    int c_pal;
...
TupleR vrepos() {
    FILE *fp = fopen ("saida/repos-final-g3.csv" , "r");
    char line[8000];
    char *string, *found, *aux;
    int c_pal;
