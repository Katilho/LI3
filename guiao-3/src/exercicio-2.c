...
static int* createArray (int *n, int fich){
    FILE *rFich;
    if(fich == 0) rFich = fopen ("saida/users-ok-g3.csv" , "r");
    else if(fich == 1) rFich = fopen ("saida/commits-final-g3.csv" , "r"); // abrimos o commits-final para comparar o repos com os commits ja filtrados
    else if(fich == 2) rFich = fopen ("saida/repos-ok-g3.csv" , "r");
...
static void remUsers () {
    FILE *rUsers = fopen ("saida/users-ok-g3.csv" , "r");
    FILE *wUsers = fopen ("saida/users-final-g3.csv" , "w");
...
static void remCommits (int arrU[], int nU, int arrR[],  int nR){
   FILE *rCommits = fopen ("saida/commits-ok-g3.csv" , "r");
   FILE *wCommits = fopen ("saida/commits-final-g3.csv" , "w");
...
static void remRepos (int arrU[], int nU, int arrC[], int nC) {
    FILE *rRepos = fopen ("saida/repos-ok-g3.csv" , "r");
    FILE *wRepos = fopen ("saida/repos-final-g3.csv", "w");
